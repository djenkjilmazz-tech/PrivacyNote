...
struct AppClipContentView: View {
    let title: String
    let encryptedPayload: String  // AES base64 (inline URL mode)
    let cloudKitToken: String     // CloudKit token (full app mode, not used in clip)
    let hasPayload: Bool
                ...
                VStack(spacing: 12) {
                    HStack(spacing: 8) {
                        Image(systemName: "eye")
                            .foregroundStyle(.orange)
                        Text("Bitirince aşağıdan notu kapat")
                            .font(.caption).foregroundStyle(.secondary)
                    }
                    Button(role: .destructive) {
                        withAnimation { step = .burned }
                    } label: {
                        Label("Okudum, Kapat", systemImage: "flame.fill")
    ...
    private var burnedView: some View {
        VStack(spacing: 20) {
            ...
            Text("Not Kapatıldı")
                .font(.title2.weight(.bold))
            Text("İçerik bu cihazda kayıtlı değil.")
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
